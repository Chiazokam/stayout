 ...
 * @returns {any} next
 */
  static async doParamsExist(param, model, res, next) {
 ...
 * @returns {any} any
 */
  static async checkParamsType(param, res, next) {
    const errors = {};
  ...
  /**
 * @description Check if parameter provided exists in the database
 * @param {object} param
 * @param {object} paramObject
 * @returns {any} next
 */
  static async doesUserExist(param, paramObject) {
    const {
      model, req, res, next
    } = paramObject;
    const paramKey = Object.keys(param);
    ...
    for (const key of paramKey) {
      for (const value of paramValue) {
        ...
        if (!paramExists) {
          errors[key] = 'Invalid credentials';
        } else {
          const {
